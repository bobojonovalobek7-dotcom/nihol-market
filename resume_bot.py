...
SUPER_ADMIN_ID = 5341602920  # <-- SIZNING ID (Super Admin)
...
def setup_database():
    with sqlite3.connect(DB_FILE) as conn:
        ...
        # Adminlar jadvali
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS admins (user_id INTEGER PRIMARY KEY, role TEXT DEFAULT 'admin', added_at DATETIME DEFAULT CURRENT_TIMESTAMP)")
        ...
        # Vakansiyalar
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS vacancies (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT NOT NULL, description TEXT DEFAULT '')")

        # Boshlang'ich vakansiyalar (XATOLIK TUZATILDI: description ham qo'shildi)
...
def generate_word_file(admin_id):
    ...
    for r in resumes:
        try:
            r_id = r[0]
            full_name = r[2]
            # Xatolik oldini olish uchun tekshiruvlar
...
async def start(message: Message, state: FSMContext):
    ...
    db_query("INSERT OR IGNORE INTO users (user_id, username) VALUES (?, ?)", (user_id, message.from_user.username),
             commit=True)
...
async def add_vacancy_handler(message: Message, state: FSMContext):
    if message.text.startswith("/"):
        ...
        # XATOLIK TUZATILDI: description uchun bo'sh joy qoldiramiz
...
async def admin_view_resumes_list(message: Message):
    ...
    resumes = db_query("SELECT id, full_name, score FROM resumes WHERE status='pending' ORDER BY created_at DESC",
                       fetchall=True)
